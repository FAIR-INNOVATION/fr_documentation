...
您可以使用布和以下清洁剂之一擦去机器人手臂上观察到的任何灰尘/污垢/油污：水、异丙醇、10% 乙醇或 10% 石脑油。如果机器人在恶劣环境下运行，例如在切削液、冷却剂等中，优傲机器人建议定期清洁或更换橡胶圈。
...
机器人手臂的额定有效负载取决于有效负载的重心偏移，其中，重心偏移定义为末端法兰中心与附加的有效负载重心之间的距离mm，可由式（1）计算。

.. figure:: installation/079.png
	:align: center
	:width: 6in

式中，分别为有效负载在末端法兰坐标系下的分量，单位为mm。

若有效负载为组合体，其中，各物体的质量分别为：m1，m2至mnkg；在末端法兰坐标系下的重心分别为：[x1 y1 z1]，[x2 y2 z2]至[xn yn zn]mm；则可通过式（2）和式（3）计算组合体的质量kg，以及重心mm。
...
负载曲线基于各机型在特定轨迹下的测试得到。其中，“完整性能”的运行环境为：各关节摩擦力补偿系数为1；各关节碰撞等级为10；Web界面设置100%运行速度及360deg/s\ :sup:`2` 加速度；动力学2.0。在此环境下，负载曲线“完整性能”部分适应绝大多数运行轨迹。

若末端负载处于“扩展后的负载能力”，须开启“时间最优模式”并满足加速度限制，或降低机器人工作范围。

FR3型号协作机器人负载曲线
*******************************

FR3型号协作机器人最大可携带负载为5kg，额定负载为3kg，负载曲线见图1所示。负载曲线的具体释义如下：
...
FR5型号协作机器人最大可携带负载为7kg，额定负载为5kg，负载曲线见图所示，为“完整性能”。
...
FR16型号协作机器人最大可携带负载为20kg，额定负载为16kg，负载曲线见图所示，为“完整性能”。
...
(2) 当负载为20kg至25kg时，为扩展后的负载能力，见“红色包络线”，此时机器人可运行在如下状态：
  ...
  ①开启“时间最优模式”，建议加速度设置在90deg/s\ :sup:`2` 以下；
...
FR30型号协作机器人最大可携带负载为35kg，额定负载为30kg，负载曲线见图所示，为“完整性能”。
