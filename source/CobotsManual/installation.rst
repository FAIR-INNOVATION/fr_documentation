...
背景
""""""""""""""""""""""""""""
宽电压控制箱给特定6.5w电磁阀供电，运行一段时间后，IO触发总时间约3~5分钟，IO端子排24V电路波动，机器人报警急停触发。

问题描述
"""""""""""""""""""""""""""""""""""""

1) 摸底验证带载7W电磁阀，测试DO输出三极管温度在158.1℃；
2) 在该温度下长时间运行存在烧坏短路风险，进而导致24V限流芯片保护；
3) 限流芯片输出为DO、强制导向继电器等供电，强制导向继电器断开后导致本体掉电；从而出现急停触发现象。

根本原因
"""""""""""""""""""""""""""""""""""""
宽压控制箱的数字输出（DO）采用推挽电路设计，下管（PNP晶体管）工作于放大区，未完全导通，导致三极管压降随负载增加而增大。

临时措施
"""""""""""""""""""""""""""""""""""""
DO输出采用继电器隔离方式控制，即通过外接40mA内小功率继电器，控制大功率电磁阀等负载；

永久措施
"""""""""""""""""""""""""""""""""""""
将DO输出推挽电路中的PNP三极管更改为PNP达林顿管，该项使推挽输出下管工作在饱和区；具体带载参数如下表。

用户手册更新内容
"""""""""""""""""""""""""""""""""""""
此次同步增加交流窄压/直流控制箱DO带载情况。在用户手册“3.5.6. 所有数字I/O的通用规范”章节末尾补充续写内容。内容如下：

数字量DO负载的电气规格如下表所示

.. centered:: 表格3.3-7 数字量DO单路负载电气规格
