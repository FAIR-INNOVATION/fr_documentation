...
.. centered:: 图表 14.2‑2 当关键参数不一致时，界面将提示错误
...
**Step1**：设置工具坐标系为“Tool0”，点击“辅助应用”->“工具应用”->“拖动锁定”，在关节扭矩传感器整机拖动模块点击功能启用，如图2-1所示。
...
**Step2**：点击“功能启用”后进行灵敏度标定，如图2-2所示，点击“生成程序”下发控制器内部Lua脚本，将机器人切换为自动模式并设置运行速度为“10”，点击“运行”，等待机器人运动。
...
**Step3**：机器人运行既定轨迹完成后，灵敏度、线性度、迟滞误差及重复性标定结果自动显示至web界面，点击“设置”应用，如图2-3所示。
...
利用双编码器（电机侧与连杆侧）的位置差进行力矩感知估计，将这个估计的力矩用于前馈补偿，机器人进行电流环拖动时减少启动力矩。
...
.. note:: 拖动增益设置范围：0-1，增益越大，补偿力矩越大，电流环启动力矩越小。
...
通过关节扭矩传感器对外力进行实时检测，当外力超出预设阈值时，机械臂会沿外力施加方向拖动；一旦外力低于力矩阈值以下，机械臂随即停止拖动，从而实现位置环控制下的拖动示教功能。
...
位置环拖动示教功能启停
""""""""""""""""""""""""""""""""""""""""""""""""""""""""
**Step1**：设置工具坐标系为“Tool0”，点击“辅助应用”->“工具应用”->“拖动锁定”，在“关节扭矩传感器整机拖动模块”中点击“功能启用”。

.. image:: application/042.png
   :width: 4in
   :align: center

.. centered:: 图表 14.5‑9 功能启用
 
**Step2**：进行零点标定，先点击“点位1”，等待机械臂运动停止后点击“记录”；再点击“点位2”，等待机械臂运动停止后点击“记录”。如果零点标定完成后关节扭矩传感器零点偏差过大，可重新进行零点标定。

**Step3**：进行灵敏度标定，点击“生成程序”下发控制器内部Lua脚本，将机器人切换为自动模式并设置运行速度为“10”，点击“运行”，等待机器人运动。待机器人运动完成后，灵敏度、线性度、迟滞误差及重复性标定结果自动显示至Web界面。灵敏度标定在机械臂每次上电时仅需执行一次。

.. image:: application/043.png
   :width: 4in
   :align: center

.. centered:: 图表 14.5‑10 零点标定及灵敏度标定

**Step4**：进入“拖动功能”，在拖动模式下拉框中选择“位置环拖动”，并设置合适的质量系数、阻尼系数、刚度系数以及力控制阈值，点击“设置”应用，即可开启位置环拖动。

.. image:: application/044.png
   :width: 4in
   :align: center

.. centered:: 图表 14.5‑11 位置环拖动参数设定

**Step5**：在拖动模式下拉框中选择“电流环拖动”，点击“设置”应用，即可关闭位置环拖动，切换为电流环拖动。

.. image:: application/045.png
   :width: 4in
   :align: center

.. centered:: 图表 14.5‑12 电流环拖动

**Step6**：若在位置环拖动模式下出现未施加外力时机械臂仍发生移动的现象，可适当增大力控制阈值。如果单轴力控制阈值增大至3Nm以上后，该现象仍未解决，则需要关闭位置环拖动，并重新执行Step2和Step4（无需重新执行Step3）。

参数具体作用及推荐值：

- **质量系数**：用于调节拖动响应，数值越大响应越慢，数值越小响应越快。1~2关节范围为[0.5-20]，3~4关节范围为[0.2-20]，5~6关节范围为[0.2-10]，推荐参数为[1, 0.8, 0.5, 0.5, 0.6, 0.6]；
- **阻尼系数**：用于调节拖动手感轻重，数值越大手感越重，数值越小手感越轻。1~2关节范围为[5-100]，3~4关节范围为[5-100]，5~6关节范围为[2-40]，推荐参数为[8, 8, 10, 10, 6, 6]；
- **刚度系数**：设置此参数后，机械臂移动到功能开启时的参考位置，推荐参数为[0, 0, 0, 0, 0, 0]；
- **力控制阈值**：位置环拖动的最小触发力。力控制阈值设置过小，可能导致机械臂非预期移动。1关节推荐力阈值大于2Nm，2关节推荐力阈值大于1Nm，3~4关节推荐力阈值大于0.8Nm，5~6关节推荐力阈值大于0.5Nm，推荐参数为[2, 1, 0.8, 0.8, 0.5, 0.5]。
