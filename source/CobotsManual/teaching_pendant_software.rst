...
-  ME工程师（初始用户名：MEenginer，密码：123）
-  PE&PQE工程师（初始用户名：PEenginer，密码：123）
-  技术员&班组长（初始用户名：programmer，密码：123）
-  操作员（初始用户名：operator，密码：123）
-  监视（初始用户名：monitor，密码：123）

用户（用户管理参考\ `4.10.2.1 用户管理 <#id67>`__\ ）默认分为六个等级，管理员无功能限制，操作员和监视员少部分功能可以使用，ME工程师、PE&PQE工程师和技术员&班组长部分功能限制，管理员无功能限制，具体默认权限参考\ `4.10.2.2 权限管理 <#id68>`__\ 。
...
点击左侧命令可以向程序树添加程序节点。程序运行时，当前执行的程序节点灰色高亮显示。在手动模式下，点击节点右侧第一个图标可以使机器人单独执行该指令，第二个图标为编辑该节点内容。
...
.. centered:: 图表 3.7‑8 N-Spiral指令界面
...
.. centered:: 图表 3.7‑10 N-Spline指令界面
...
.. centered:: 图表 3.7‑16 ToolList指令界面
...
.. centered:: 图表 3.7‑20 if…else指令界面
...
.. centered:: 图表 3.7‑22 WaitTime指令界面
...
.. centered:: 图表 3.7‑24 WaitMultiDI指令界面
...
.. centered:: 图表 3.7‑29 Weave配置与测试指令界面
...
.. centered:: 图表 3.7‑30 Segment指令界面
...
.. centered:: 图表 3.7‑32 Gripper指令界面
...
.. centered:: 图表 3.7‑37 Conveyor指令界面
...
.. centered:: 图表 3.7‑42 W-Search指令界面
...
.. centered:: 图表 3.7‑43 W-Search示例（1D）
...
.. centered:: 图表 3.7‑46 ServoCart指令界面
...
.. centered:: 图表 3.7‑47 Function指令界面
...
.. centered:: 图表 3.7‑48 Collision指令界面
...
.. centered:: 图表 3.7‑61 局部示教点详情

当前程序备份
~~~~~~~~~~~~~~

用户修改当前程序后，触发当前程序的“备份”功能(备份时间为1年)，将当前程序的初始内容进行保存展示在右侧，方便用户对比修改的内容。
...
.. centered:: 图表 3.7‑64 图形化编程页面布局整体设计
...
-  **安全墙配置**：点击启用按钮，可启用的相应的安全墙。当安全墙未配置安全范围时，会提示报错。点击下拉框，选择想要设定的安全墙，自动带出安全距离(可不设置，默认位0),再点击“设置”按钮，设置成功。
  
.. image:: teaching_pendant_software/190.png
   :width: 6in
   :align: center

.. centered:: 图表 3.9‑33 安全墙配置

-  **安全墙参考点配置**：选择安全墙后，可设置四个参考点。前三个点为平面参考点，用来确认设置的安全墙的平面。第四个点为安全范围参考点，用来确认设置的安全墙的安全范围。

-  注：若参考点设置成功，则亮绿灯。反之，则亮黄灯。直到参考点设置成功后转为绿灯。当四个参考点都设置成功后，可计算其安全范围，计算成功后安全范围参数点状态恢复默认。

.. image:: teaching_pendant_software/191.png
   :width: 6in
   :align: center

.. centered:: 图表 3.9‑34 安全范围参考点设置

-  现象：启用配置成功的安全墙。拖动机器人，若处在设定安全范围内，则系统正常。若处在设定安全范围之外，则提示报错。

.. image:: teaching_pendant_software/192.png
   :width: 6in
   :align: center

.. centered:: 图表 3.9‑35 安全范围设置成功后效果图
...
启用安全后台程序并设置意外情况场景和后台程序，当用户开始运行程序，发生的意外情况场景与设置的意外情况匹配时，机器人会停止当前程序的运行并立马执行相对应的后台程序，起到安全防护的作用。

.. image:: teaching_pendant_software/daemon.png
   :width: 6in
   :align: center

.. centered:: 图表 3.9‑36 安全后台程序
...
点击二级菜单栏账户设置，进入账户设置界面。账户管理功能仅限超级管理员和管理员可使用。功能分以下三个模块：

用户管理
++++++++++++++++

用户管理页面，用来保存用户信息，可以添加用户的工号、职能等。用户可通过已有的用户列表进行手动登录或者刷卡登录操作。
...
-  **新增用户**：点击“新增”按钮，输入工号、姓名、密码并选择职能。注：工号为10位整数型，工号和密码都有唯一性校验，且密码通过盲文显示。用户新增成功后，可以输入姓名和密码进行重新登录。
...
-  **删除用户**：删除方法分为单条删除和批量删除。1.点击列表右侧单条“删除”按钮，提示“请再次点击删除按钮以确认删除”，再次点击该列表删除成功。2.点击左侧复选框，选择需要删除的用户，再点击列表上方批量“删除”按钮两次后删除。注：初始用户111以及当前登录用户无法删除。
...
参数范围配置，只有管理员可进行参数范围的调节，超级管理员只读，其他权限成员的参数只可在管理员设定的参数范围之内设定。

参数设定方式分为两种：滑块拖动和手动输入。

注：参数范围最大值必须大于最小值。参数范围配置成功3秒后，自动跳转到登录页，需重新登陆。
