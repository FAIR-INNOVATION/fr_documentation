...
**1.Modbus主站设置**：在页面配置主站和对应主站寄存器地址。可进行“添加Modbus主站、添加主站寄存器”相关操作。

**主站信息**：主站信息包含主站名称、从站ip、端口号、从站号和通信周期，可进行编辑、刷新和删除操作。

**主站寄存器信息**：主站寄存器信息包含类型（DI、DO、AI、AO）、地址编号、名称和地址值(DI、AI类型时不可输入)，可进行编辑和删除操作。
...
**2.Modbus从站设置**：可进行数字输入、数字输出、模拟量输入、模拟量输出以及别名修改操作。
...
**数字输入/输出**：数字输入DI只读，数字输出DO点击后可控制指示灯开关，且个数都为128个。

**模拟量输入**：模拟量输入AI分为无符号、有符号和浮点型号(三位小数)，都为只读。其中无符号和有符号个数为16个，浮点型为32个。

**模拟量输出**：模拟量输出AO分为无符号、有符号和浮点型号(三位小数)，其中参数范围，无符号：0~65535,有符号：-32767~32768，浮点型：最多三位小数。其中无符号和有符号个数为16个，浮点型为32个。

**3.从站别名设置**：点击从站别名，可进行编辑操作，修改从站别名。
