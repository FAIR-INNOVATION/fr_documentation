...
增加Lua开放设计，同时兼容之前已有设计，厂商内容为：厂商=0x60，类型=0x00，代表FR Lua开放接口并选择对应外设厂商及类型配置。

操作步骤
~~~~~~~~~~~
**Step1**：进入机器人本体->系统升级界面，选择末端固件.bin文件，升级末端固件。
...
**Step2**：进入用户外设配置->Lua开放协议页面，上传Lua末端开放协议，选择需要上传的Lua末端开放协议，进行上传操作。
...
**Step3**：配置末端外设，设备类型为：力传感器设备，厂商为：OpenLua，然后选择软件版本和挂载位置，点击“配置”按钮。

.. figure:: robot_peripherals/148.png
   :align: center
   :width: 6in

.. centered:: 图表 5.1‑6 末端外设配置

**Step4**：配置末端通讯参数，通讯参数包含波特率、数据位、停止位等，配置完成后，点击“设置”按钮。

.. figure:: robot_peripherals/149.png
   :align: center
   :width: 6in

.. centered:: 图表 5.1‑7 配置末端通讯参数
...
.. centered:: 图表 5.1‑10 力/扭矩传感器原始数据
