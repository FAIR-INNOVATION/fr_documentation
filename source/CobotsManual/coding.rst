...
协作机器人电弧跟踪焊接过程中，焊机将实时焊接电流信号反馈至PLC，再由PLC通过UDP通讯反馈至机器人；机器人根据实时反馈焊接电流数值进行焊接轨迹位置补偿，实现电弧跟踪效果。上述焊机和PLC之间电流信号反馈有两种方式：
...
②IO模拟量：PLC也可以直接采集模拟量信号，再将模拟量信号以一定的转换关系转换成电流数值反馈至机器人；若您的焊机具有实时焊接电流模拟量输出通道，可直接将该通道接入至PLC的模拟量输入模块中；若您的焊机不具有实时焊接电流模拟量输出通道，可外接一个霍尔电流传感器，传感器采集实时焊接电流信号，并将焊接电流信号转成模拟信号输出至PLC模拟量输入模块。
...
依次点击“初始设置”、“基础”、“I/O设置”、“AI”，根据实际配置选择相应的扩展AI通道，点击“应用”按钮。

.. note:: 
   机器人与PLC间UDP通讯协议见《8.5.5 附件一：机器人UDP通讯协议》，协议中PLC向机器人反馈数据包含序号70~73的4个模拟输入通道，对应图4-4中的Aux-AI0~3；

   焊接过程中PLC采集实时焊接电流信号，并将其转成0~4095数值信号赋值给某个模拟量输入通道，默认为“Aux-AI0”，机器人端采集相应模拟量输入通道数值进行电弧跟踪操作；

   若您需要更改电弧跟踪通道为“Aux-AI1”、“Aux-AI2”或“Aux-AI3”，需要同步更新PLC端程序，将采集的实时焊接电流赋值给相应的模拟量输入端口）。

.. image:: coding/282.png
   :width: 4in
   :align: center

.. centered:: 图表 9.8-7-4 电弧跟踪通道配置
...
.. centered:: 图表 9.8-7-6 设置倾斜摆动轨迹（左），当符号正确时焊缝轨迹（右）
...
**Step2**：点击上方“新建”按钮，输入“example”，选择“empty.lua”，建立新的lua脚本，如图1所示。
...
点击“Modbus_RTU” ，选择“从站”，打开ModbusRTU从站指令添加页面(图60)。。
...
模拟输出(输入寄存器)、模拟输入(保持寄存器)的读写操作与数字输出(离散输入)、数字输入(线圈)的操作基本一致，区别在于后者的数据范围仅限于0或1，而前者的数据范围更大，因此具体的操作可参考数字输出和数字输入程序的编写，在此仅展示模拟输入的读操作(图3-24)和模拟输出的读写操作(图3-25)程序示例。
...
**Step1**：点击“初始设置”->“基础”-> “坐标系”->“工具”，进入工具坐标系设置界面，选择“坐标系名称”并设置末端工具对应的坐标系参数。
...
.. centered:: 图表 9.33-2 输入百分比显示实际物理速度值
...
过渡点和终点都可以对是否偏移进行设置，可以选择基于基坐标系偏移和基于工具坐标偏移，并弹出x，y，z，rx，ry，rz偏移量设置，终点可以设置平滑过渡半径，实现运动连续效果。(圆弧运动需要先添加PTP或者Lin指令先运动到起始点)。

.. image:: coding/460.png
   :width: 6in
   :align: center

.. centered:: 图表 9.33-4 Arc指令界面
...
.. centered:: 图表 9.33-5 输入百分比显示实际物理速度值
...
.. centered:: 图表 9.33-8 起始点运动方式和起始点“P1”
...
依次配置A-E键位和IO键。Smart Tool配置完成后，机器人内部维护每个按钮对应的功能，当检测到某按钮被按下时，自动执行该按钮对应功能项。
...
.. centered:: 图表 9.33-13 输入百分比显示实际物理速度值
...
.. centered:: 图表 9.33-15 Smart Tool配置（A-E键位）
...
.. centered:: 图表 9.33-16 Smart Tool配置（IO键位）
...
**Step1**：登录web界面，依次点击“初始设置”→“基础”→“关节”→“碰撞等级”，进入机器人碰撞等级设置模块，设置合理的碰撞系数，见图2-1所示。
...
**Step3**：依次点击“示教程序”→“程序编程”→“力控集”，添加“Impedance”指令。“Impedance”指令可以使机器人在运行轨迹上实现阻抗控制（目前，只开放了笛卡尔空间下的阻抗控制）。
