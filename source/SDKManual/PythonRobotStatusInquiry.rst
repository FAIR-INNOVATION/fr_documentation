...
.. csv-table:: 
    ...
    - 错误码 成功-0  失败- errcode
    - ``coord``：坐标系数值
    - ``type``：工具类型 (可选) 0-工具；1-传感器
    - ``install``：安装位置 (可选) 0-机器人末端；1-机器人外部
    - ``toolID``：工具ID
    - ``loadNo``：负载编号
    "
...
.. csv-table:: 
    ...
    - 错误码 成功-0  失败- errcode
    - ``coord``：坐标系数值
    - ``tcoord``：机器人末端安装工件坐标系位姿 (6个值: [x, y, z, rx, ry, rz])
    "
...
.. csv-table:: 
    ...
    - 错误码 成功-0  失败- errcode
    - ``coord``：坐标系数值
    - ``axisCoordNum``：扩展轴号 (可选) bit0-bit3对应扩展轴1-扩展轴4；如值为3,对应应用扩展轴[1,2]
    - ``calibFlag``：标定标志 (可选) 0-未标定；1-已标定
    "
